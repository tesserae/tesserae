	<?php include "first.php"; ?>

				<div id="nav_sub">
					<ul>
						<li><a href="<?php echo $url_html.'/index.php' ?>">Search Home</a></li>
						</ul>
				</div>
			</div>
	
			<div id="main">

				<h1>People</h1>
				
				<h2>Principal Investigators</h2>
				<p>
					<strong>Neil Coffee</strong>
					is the chair of the Department of Classics, University at Buffalo.
				</p>
				<p>
					<strong>J.-P. Koenig</strong>
					is the chair of the Department of Linguistics, University at Buffalo.
				</p>
				</table>

				<h2>Version 1</h2>
				<p>
					Shakthi Poornima<br />
					Chris Forstall
				</p>

				<h2>Version 2</h2>
				<p>Roelant Ossewaarde</p>
				
				<h2>Version 3</h2>
				<p>
					Chris Forstall <br />
					Xia Lu
				</p>

				<h2>Development Teams</h2>

				<h3>Corpus</h3>
				<p>
					Katie Lamberto<br />
					Kevin Roth<br />
					Jacob Billings<br />
					Tina Tetter
				</p>

				<h3>Scoring</h3>
				<p>
					James Gawley<br />
					Jenny Krantz<br />
					Christina Caucci<br />
					Katie Roache<br/ >
					Jia Song
				</p>

				<h3>Semantics</h3>
				<p>
					Sam McCabe<br />
					Derry Moore<br />
					Sarah Skelley<br />
					Hao Sun
				</p>

				<h2>Collaborators</h2>
				<p>
					<strong>Walter J. Scheirer</strong><br />
					University of Colorado at Colorado Springs
				</p>

				<h2>Advisory Board</h2>
				<p>
					<strong>David Bamman</strong><br />
					Carnegie Mellon University
				</p>
				<p>
					<strong>Marco B&uuml;chler</strong><br />
					Universit&auml;t Leipzig
				</p>
				<p>
					<strong>Gregory Crane</strong><br />
					Tufts University
				</p>
				<p>
					<strong>Stephen Hinds</strong><br />
					University of Washington
				</p>
			</div>

			<?php include "last.php"; ?>
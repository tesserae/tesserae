...
print join("\t", qw/name tokens lines phrases lemmata/) . "\n";

my @lang = grep {/la|grc/} @ARGV;

for my $lang (@lang) {

	my @list = @{Tesserae::get_textlist($lang, sort=>1)};
	
	for my $name (@list) {
	
		my $base = catfile($fs{data}, 'v3', $lang, $name, $name);
		
		my $tokens  = scalar(@{retrieve("$base.token")});
		my $lines   = scalar(@{retrieve("$base.line")});
		my $phrases = scalar(@{retrieve("$base.phrase")});
		my $lemmata = scalar(keys %{retrieve("$base.index_stem")});
		
		print join("\t", $name, $tokens, $lines, $phrases, $lemmata) . "\n";
	}
}

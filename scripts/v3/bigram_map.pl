...
for my $unit (qw/phrase/) {
	...
	for my $text (@corpus) {
		...
		#if add_multi.pl hasn't been run on this text, skip it.		
		push (@not_indexed, $file_index_stem) and next unless -e $file_index_stem;
		
		# get the unit ids of the phrases here 
		my $file_index_phrase = catfile($fs{data}, 'v3', $lang, $text, $text . ".phrase");
		
		push (@not_indexed, $file_index_stem) and next unless -e $file_index_phrase;
		
		my %index = %{retrieve($file_index_stem)};
		
		my @loci = @{retrieve($file_index_phrase)};
		
		while (my ($key, $value) = each %index) {
		
			#retrieve the hash kept under this bigram key
			#the keys of that hash are unit ID numbers for the phrases containing bigram
			my @units = keys %{$index{$key}};
			
			#grab the LOCUS	for each bigram
			foreach my $unit_id (@units) {
				
				my $locus = $loci[$unit_id]->{'LOCUS'};
				
				push (@{$count_corpus{$key}}, "$text $locus"); 
	...
	my $file_freq = catfile($fs{data}, 'common', "$lang.bigram_map_${unit}_stem");
	
	nstore \%count_corpus, $file_freq;
}
print "\n";
foreach my $skipped (@not_indexed) {

...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="/">
    ...
    <xsl:text>&quot;</xsl:text>
    <xsl:value-of select="results/@source"/>
    <xsl:text>&quot;</xsl:text>
<xsl:text>
</xsl:text>
    ...
    <xsl:text>&quot;</xsl:text>
    <xsl:value-of select="results/@target"/> 
    <xsl:text>&quot;</xsl:text>
<xsl:text>
</xsl:text>
...
<xsl:text>
    ...
    <xsl:text>"Matched On","Target Loc","Source Loc","Target Phrase","Source Phrase","Score"</xsl:text>
<xsl:text>
</xsl:text>

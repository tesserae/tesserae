...
while (my $file_in = shift @ARGV)
{
	...
	unless (-d "$fs_data/db/$lang/word") { make_path("$fs_data/db/$lang/word") }

	my $file_out = "$fs_data/db/$lang/word/$name";
